...
for v,s in vstations.items():
    for j in x:
        if j["station_id"] == v:
            print(s)
            print("Vélib mécanique(s): " + str(j["num_bikes_available_types"][0]["mechanical"]))
            print("Vélib électrique(s): " + str(j["num_bikes_available_types"][1]["ebike"]))
            print("Place(s): " + str(j["numDocksAvailable"]))
            print("")
